...
class NetatmoCamera(NetatmoModuleEntity, Camera):
    ...
    def __init__(
        ...
        self._home_signal_name = f"{HOME}-{self.home.entity_id}"
            ...
            [
                {
                    ...
                    SIGNAL_NAME: self._home_signal_name,
                },
                {
                    "name": EVENT,
                    "home_id": self.home.entity_id,
    ...
    def fetch_persons(self, person_names: list[str | None]) -> list[NaPerson]:
        ...
        for name in person_names:
            ...
            for pid, person in self.home.persons.items():
                if person.pseudo == name:
                    persons.append(person)
                    person_id = pid
                    break

            if person_id is None:
                person_id_errors.append(name)
    ...
    async def _service_set_persons_home(self, **kwargs: Any) -> None:
        ...
        self.data_handler.notify(self._home_signal_name)

    async def _service_set_person_away(self, **kwargs: Any) -> None:
        """Service to mark a person as away or set the home as empty."""
        name = kwargs.get(ATTR_PERSON)
        persons = self.fetch_persons([name] if name else [])
        person = next(iter(persons), None)
        person_id = person.entity_id if person is not None else None
